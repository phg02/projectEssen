...
const form = document.querySelector('#imageDrop');
form.addEventListener('submit', function(e) {
	...
	if (!fileInput.files.length) {
        ...
        if (!validImageTypes.includes(fileType)) {
            ...
            fileInput.classList.add('error-input');
            fileInput.focus();
            errorList += '<li>File must be a PNG or JPEG image.</li>';
        }
    }

	
	let errors = "<ul class='error-list' class='error-list'>" + errorList + "</ul>";
	if (error) {
		Swal.fire({
			icon: "error",
			title: "Oops...",
